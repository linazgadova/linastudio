 ...
 * бы смысл.
 *
 * Здесь нет ни одного обработчика скролла и ни одного
...
function measure() {
  ...
  layout = stops
    .map((s) => {
      ...
      return {
        ...
        // Зона покоя вокруг середины секции — короткая. Чем она
        // длиннее, тем меньше остаётся дороги между секциями и тем
        // круче приходится идти по ней: весь переход сжимается в
        // короткий участок прокрутки и читается броском. Короткий
        // покой отдаёт почти всю высоту секции под движение, и то же
        // расстояние шар проходит втрое спокойнее
  ...
  /*
    Первая точка владеет первым экраном.

    Отсчёт идёт от середины окна, а середина короткой шапки выше её.
    Поэтому при нулевой прокрутке страница оказывалась уже на полпути
    к следующей секции: шар открывался не там, где его поставила
    шапка, а посреди колонки текста под заголовком. Сдвигаем первую
    точку вниз до середины экрана — весь путь от неё начинается
    ровно тогда, когда человек тронул колесо.
  */
  if (layout.length) layout[0].y = Math.max(layout[0].y, window.innerHeight / 2)
