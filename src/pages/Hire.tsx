...
export function Hire() {
   ...
   * экрана: из центра вправо, оттуда влево и обратно в центр. Здесь
   * он держится правой половины и низа: верх справа, потом выход
   * за правое поле, потом низ по центру и снова низ справа.
   *
   * Текст на этой странице идёт во всю ширину — таблица на девять
   * колонок, две колонки метода, три колонки кода. Разойтись с ним
   * совсем негде, поэтому там, где шар достаёт до строк, под ними
   * лежит размывка бумаги: hire__veil в стилях, а у колонки стека
   * своя, размером с ячейку.
   */
  useAuroraSection(
    headRef,
    {
  ...
  useAuroraSection(tableRef, {
    cx: 1.0, cy: 0.42, radius: 0.34, form: 0.5, intensity: 0.9, shift: 0.34,
    accent: '#5B3BE8', tint: 0.55,
  })
  useAuroraSection(howRef, {
    cx: 0.5, cy: 0.26, radius: 0.3, form: 0.62, intensity: 0.9, shift: 0.62,
  ...
  useAuroraSection(endRef, {
    cx: 0.86, cy: 0.26, radius: 0.44, form: 0.45, intensity: 0.95, shift: 0.88,
    accent: '#FF2D6F', tint: 0.5,
  })
